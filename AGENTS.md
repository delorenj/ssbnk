...
- `remote-screenshot-upload.sh`: Reference script for remote machines. Uses inotifywait (Linux) or fswatch (macOS) to watch for new screenshots and POST them to the upload endpoint.
...
- The watcher container mounts `./web/html` as `/data/hosted`. Container paths like `/data/hosted/file.png` don't exist on the host.
